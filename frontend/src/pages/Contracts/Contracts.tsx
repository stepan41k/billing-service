...
// Анимация раскрытия карточки — spring с небольшим bounce
const bodyVariants = {
  collapsed: { height: 0, opacity: 0 },
  expanded:  {
    height: 'auto', opacity: 1,
    transition: { height: { duration: 0.28, ease: [0.25, 0.46, 0.45, 0.94] }, opacity: { duration: 0.2 } },
  },
};
...
const ContractCard = memo(function ContractCard({ c, activeContract, onConnect }: CardProps) {
  ...
  return (
    <motion.div
      ...
      layout
    >
      <button className={styles.header} onClick={() => setOpen((p) => !p)}>
        <div className={styles.headerLeft}>
          <span className={styles.number}>№ {c.number}</span>
          <span className={`${styles.badge} ${styles[c.status]}`}>{STATUS_LABEL[c.status]}</span>
      ...
      >
        <div className={styles.grid}>
          ...
          <div className={styles.field}>
            <span>Трафик</span>
            <strong>{c.unlimitedTraffic ? 'Безлимит' : `${c.trafficLimitGb} ГБ/мес`}</strong>
          </div>
          <div className={styles.field}>
            <span>Стоимость</span>
...
export default function Contracts() {
  ...
  const handleConnect  = useCallback((c: Contract) => setPending(c), []);
  const handleClose    = useCallback(() => setPending(null), []);
  const handleConfirm  = useCallback(() => {
    // TODO: api.switchTariff(pending!.id)
    setPending(null);
  }, [pending]);
  ...
  return (
    <>
      ...
      <Modal open={Boolean(pending)} onClose={handleClose} onConfirm={handleConfirm}>
        <p>
