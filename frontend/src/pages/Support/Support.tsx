    ...
    <PageTransition>
      <div className="space-y-6">
              ...
              <Card>
                <CardContent className="p-4">
                  <form onSubmit={onCreateSubmit} className="space-y-3">
                    <Input placeholder="Тема" value={newSubject} onChange={e => setNewSubject(e.target.value)} required />
                    <Input placeholder="Опишите проблему" value={newText} onChange={e => setNewText(e.target.value)} required />
        ...
        <div className="space-y-3">
                      ...
                      <CardContent className="border-t border-border pt-4 space-y-3">
                        {t.messages.map(m => (
                          <div
                            ...
                            className={cn(
                              ...
                              m.sender === 'user' ? 'bg-primary/5 ml-6' : 'bg-muted mr-6'
                            )}
                          >
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-xs font-medium text-muted-foreground">
                                {m.sender === 'user' ? 'Вы' : 'Поддержка'}
                              </span>
                        ...
                        {t.status !== 'closed' && (
                          <div className="flex gap-2">
                            ...
                            />
                            <Button size="icon" onClick={() => onReply(t.id)}>
                              <Send size={14} />
                            </Button>
                          </div>
                        )}
          ...
          {!tickets.length && <p className="text-sm text-muted-foreground">Тикетов пока нет</p>}
        </div>
      </div>
    </PageTransition>
  )
