              ...
              <CardContent className="flex items-center gap-3 p-4">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-success">
                    {latest.downloadMbps.toFixed(1)} <span className="text-xs font-normal">Mbps</span>
                  </p>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="flex items-center gap-3 p-4">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-primary">
                    {latest.uploadMbps.toFixed(1)} <span className="text-xs font-normal">Mbps</span>
                  </p>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="flex items-center gap-3 p-4">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-warning">
                    {latest.pingMs} <span className="text-xs font-normal">ms</span>
                  </p>
                </div>
              </CardContent>
            </Card>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border text-left text-xs text-muted-foreground">
                        ...
                        <th className="pb-2 pr-4">↓ Mbps</th>
                        <th className="pb-2 pr-4">↑ Mbps</th>
                    ...
                    <tbody>
                      {results.map((r: SpeedTestResult) => (
                        <motion.tr key={r.id} variants={item} className="border-b border-border/50 last:border-0">
                          <td className="py-2.5 pr-4 text-muted-foreground">{formatDateTime(r.createdAt)}</td>
                          ...
                          <td className="py-2.5 pr-4 font-medium text-warning">{r.pingMs}</td>
                          <td className="py-2.5 text-foreground">{r.server}</td>
                        </motion.tr>
                      ))}
                    </tbody>
                  </table>
                  {!results.length && <p className="text-sm text-muted-foreground">История пуста</p>}
                </div>
              </motion.div>
            )}
          </CardContent>
